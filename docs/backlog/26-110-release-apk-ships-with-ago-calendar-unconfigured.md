...
- **Stage**: 26
- **Status**: ready — a one-line CI build-config fix; the direction is decided (see below).
...
- [ ] The CI release build passes `-PagoCalendarApiBaseUrl=https://calendar-api.reserve-me.ru`; the
      published APK's `AGO_CALENDAR_API_BASE_URL` is that origin (not empty).
