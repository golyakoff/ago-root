...
| `operators` (**AGO Calendar**) | `id`, `tenant_id`, `display_name`, `external_subject_id?`, **`invited_email?` (`20-08`)**. **A display name, unlike AGO Chat's `operators`, which holds none** — worth noticing rather than discovering later: the two products' operator tables are not the same shape, and this one names a person. **`20-08` made it a *direct identifier* rather than only a name**: `invited_email` is the address a tenant typed when inviting a colleague (`adr/0088`), used once by `OperatorIdentityClaimsTransformation` to match a Keycloak email claim against not-yet-linked rows. **Deliberately not cleared after linking** — it is the record of who the invite was for, which is the audit trail the linking flow's own accountability rests on; clearing it would make "who invited whom" unanswerable the moment it succeeded | AGO | Forever. No pruning exists | Row deletion; `ON DELETE CASCADE` from `tenants`. Nothing clears `invited_email` independently of the row | `Stage20CreateCalendarSchema.cs`; `Stage20AddOperatorInvitedEmail.cs`; `Ago.Calendar.Domain/Operator.cs`; `Ago.Calendar.Domain/InvitedEmail.cs` |
