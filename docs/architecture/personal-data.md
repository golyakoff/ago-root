...
|---|---|---|---|---|---|
| `messages.body` (Postgres) | Free text; anything a visitor or operator typed | AGO, on its own node (`adr/0026`) | **Per tier, per `adr/0031`/`13-06`/`23-74`.** The class a message was written under (`retention_class`, immutable) determines its window, set once at write time and never recomputed. **Solo (free): 2 months** (`13-08`). **Business (starter/growth): forever, while the tier stays paid** (`23-74`/`ago-business/0012` §5 — the disk-safety concern that used to cap every class at an operational ceiling is now met by a per-tenant storage *quota*, not a time window; `23-52`/`23-80`/`23-82`). Once past the window **the row moves to the archive below, not to nothing**. **A downgrade destroys nothing already written while paid** — `adr/0031`'s Decision 2 promised this and `RetentionClassImmutabilityTests` now proves it: the stamped class survives a later change to the site's own tier | Row deletion via `ON DELETE CASCADE` (a live conversation still deleted this way); at the retention horizon, `MessagePartitionPruneJob` `DROP`s the whole month's partition for that class **only after `MessageArchiveJob` has confirmed a matching archive object exists** (`MessageArchiveGate`) | `AgoChatDbContextModelSnapshot.cs`; `Stage13RepartitionMessagesByRetentionClass.cs`; `MessagePartitionPruneJob.cs`; `MessageArchiveGate.cs`; `MessagePartitionPruneJobOptions.cs` (`23-74`); `RetentionClassImmutabilityTests.cs` (`23-74`) |
...
| `attachments` rows + the MinIO objects they point at | Attachment bytes — a photo, a document, a screenshot — plus content type and size. **Since `23-82`**, also `download_count`/`last_downloaded_at` — how many times, and when last, a presigned GET for this attachment was freshly issued (a proxy for real downloads, not a byte-exact count — `GetAttachmentDownloadUrlHandler`'s own remarks). A fact about who was shown this file and when, not a new column with its own lifetime: it rides the same row, so it cascades and erases exactly where the row already does, named here rather than left implicit | AGO | **Follows the owning message's window, per `13-06`/`adr/0031`'s Decision 4** — an attachment is swept in the same call that drops the one partition whose rows referenced it (`MessagePartitionPruneQuery.ListReferencedAttachmentIdsAsync`, read before the drop; a date-range predicate alone cannot substitute — a site that changes tier mid-month has attachments belonging to two different partitions in the same calendar month, see `adr/0074`). Before that, unchanged: **Forever, no retention job** | `DeleteAttachmentHandler` deletes the object *and* the thumbnail; `5-04`'s sweeper deletes only objects whose row never got a `message_id`; `MessagePartitionPruneJob`'s own sweep (`13-06`) deletes an expired attachment's object and thumbnail via `AttachmentRetentionSweepQuery`, reusing that same delete-then-clean-up-storage shape. **The pre-existing gap is unchanged**: deleting a conversation directly still cascades `attachments` rows and leaves MinIO objects behind | `DeleteAttachmentHandler.cs:55-95`; `AttachmentOrphanSweepJob`; `MessagePartitionPruneJob.cs`; `AttachmentRetentionSweepQuery.cs`; `Stage5AddAttachments.cs` cascade FKs |
| `message_archives` (Postgres manifest) + the archive `.zip` objects in object storage, prefix `archive/messages/{siteId}/{class}/{period}.zip` | **A copy** of every message and attachment reference (metadata + a presigned download URL, never bytes) from one site's one retention-class partition for one month, in `16-03`'s own export format | AGO, same store as every other object-storage entry (no distinct storage class/bucket yet — `15-02`'s own open vendor question, unchanged) | **Indefinite** — the destination once the live row's own window ends, not itself time-boxed. `adr/0031`: "archiving moves the liability, it does not end it" | **Closed by `24-09`.** A conversation's own erasure request now reaches this store too, not only the live row: `ConversationErasureJob`'s own `ConversationArchiveEraser` step downloads every archive object the site has, drops the `messages.jsonl`/`attachments.jsonl` lines naming the erased conversation, and re-uploads the result to the same key — a rewrite, not a delete, because one object covers every conversation a site had in one period and deleting it whole would destroy every other visitor's transcript archived alongside it (`docs/adr/0108-*`). A whole-**site** erasure reaches the object too: `SiteErasureJob` deletes each of the site's archive objects outright, once `ConversationErasureJob`'s own draining has already stripped every one of them down to an empty shell — closing the "the `.zip` is then orphaned in storage" gap this row itself used to name for that path. The manifest row still cascades with the site as before; the object it names is no longer left behind. **This changes conversation erasure's own reliability profile**: it now depends on object storage being reachable for a read as well as a write, and can take one round trip per archived period the site has | `MessageArchiveWriter.cs`; `MessageArchiveJob.cs`; `MessageArchiveRepository.cs`; `Stage13AddMessageArchives.cs`; `ConversationArchiveEraser.cs`; `ConversationErasureJob.cs`; `SiteErasureJob.cs` (`24-09`) |
...
| `visitors` | `id`, `site_id`, `first_seen_at`, `last_seen_at`. **`25-56`, added**: `emoji_creature`/`emoji_food` — two emoji, each drawn once from a fixed twenty-member shipped list (`VisitorEmojiDictionary`) and never reassigned, purely as an operator-facing mnemonic beside the conversation-switching panel's short code. **Not personal data in any sense this table's own rule tracks** — the pick is uniform-random at assignment time and carries no information about the visitor at all (unlike `conversations`' traffic-source columns two rows below, which at least *echo* something a real browser sent); it is listed here only so this row's own "and nothing else" stays literally true. Rides the same single retention rule as every other column here rather than needing one of its own, the identical reasoning `conversations`' own four nullable columns two rows below already state for themselves | AGO | Forever | Row deletion; cascades from `sites` | `Stage1CreateChatSchema.cs:69-86`, `Stage25AddVisitorEmojiPair.cs` (`25-56`), `Visitor.cs` |
...
| `visitor_contact_details` (`14-14`) | A phone number, an email address, or a short "work mobile"-style annotation an operator typed because a visitor said it out loud — or, **since `23-09`**, a number or address the **visitor themselves** typed into the widget's own control when nobody was on shift (`decisions.md` §4). Still **never** because a real message arrived from that address and **never** because a verification code proved anyone controls it - that remains `ChannelIdentity`'s meaning and this table's is deliberately weaker (`adr/0128`). Which of the two it is, is a column: `source` (`Operator`/`Visitor`), never inferred from `recorded_by_operator_id` being null. `verified` is a second column that **starts false and has no writer that sets it true** - verification means somebody other than the claimant established control, and a flag a subject could set about themselves would be worse than none because a reader trusts it. **Since `25-58`**, a third column, `assessment` (`Unset`/`Confirmed`/`Invalid`), holds an operator's own overridable opinion of whether a Phone/Email row is still good - never `verified` restated under a new name (`VisitorContactDetailAssessment`'s own remarks hold the two apart deliberately), and **since the same item**, `value` itself is editable in place by an operator (`VisitorContactDetail.EditValue`) rather than write-once - an edit never touches `source`/`recorded_by_operator_id`, so a visitor's own submission edited by an operator stays attributed to the visitor. The same directly-identifying class `customers` holds for AGO Calendar, this table's own row above calls "the most directly identifying store either product has" | AGO | **Indefinite — the tenant's own asset, not a timed record of the visitor's.** Corrected 2026-09-04 (`docs/design/decisions.md` §4): the ninety days first proposed for this row were never agreed and are wrong. The contact ends when the person asks for erasure or when the tenant judges it useless — it carries no timer of its own. **Two clocks, stated as two, and neither may later collapse into the other**: this row's own retention is independent of `messages.body`'s per-tier window two rows above — a transcript ages out and archives on `adr/0031`/`13-06`'s schedule while the contact standing beside it does not, or the tenant would lose an asset every time a transcript's own window closed | The tenant's own per-row action — `DELETE /api/v1/conversations/{conversationId}/contact-details/{id}` (`DeleteVisitorContactDetailHandler`, gated on `Permission.ConversationSend`) — is what decision 4's "the tenant needs an action to say so" points at; it already existed before this row did, and the endpoint itself is untouched. **Since `25-58`, the console's own casual per-row delete button is gone** - the panel's investigated conclusion was that a wrong value now gets corrected (edit) or flagged (`assessment` = `Invalid`) instead, strictly more informative than removing the row outright, so an operator's ordinary "this is no longer good" case is served by marking it invalid, not by deleting it. **This is a real, open question the panel's own removal does not itself resolve: whether decision 4's "the tenant judges it useless" retention trigger still has a reachable console action, now that its only prior UI path is gone**, or whether that judgment is meant to route through `assessment = Invalid` from here on (a state, not a removal) - stated here rather than silently assumed, because the answer changes what "the tenant needs an action to say so" now points at. The `DELETE` route itself remains reachable through the API directly, unreached by any console click today. **Since `23-08`**, a person's own erasure request takes it too: erasing a conversation drains that visitor's contact details explicitly (`ConversationErasureQuery.DeleteContactDetailsForVisitorAsync`, the same "primary mechanism is explicit, cascade is defence in depth" treatment `DeleteNotesForConversationAsync`/`DeleteTagsForConversationAsync` already get) — "a person's erasure request takes the conversation and the contact, it is all their data" (`decisions.md` §4). `ON DELETE CASCADE` from `visitors` (and from there, `sites`) remains the backstop for a stray row either sweep somehow missed, unchanged. **A retention sweep of `messages` never reaches this table at all** — `MessagePartitionPruneJob` reads and writes only `messages`, a guard proven by `MessagePartitionPruneJobTests.PruneAsync_RemovesTheExpiredMessage_ButLeavesTheVisitorsContactDetailsStanding`, which is what stands between this row's "indefinite" and a future retention change quietly making it timed | `Stage14AddVisitorContactDetails.cs`; `VisitorContactDetailConfiguration.cs`; `VisitorContactDetail.cs`; `ConversationErasureQuery.cs`/`ConversationErasureJob.cs` (`23-08`); `Stage25AddVisitorContactDetailAssessment.cs`, `EditVisitorContactDetailHandler.cs`, `SetVisitorContactDetailAssessmentHandler.cs` (`25-58`) |
| `conversations` | The visitor↔operator pairing, timestamps, unread counters. **`18-12`, added**: an optional traffic source — the referring page's host, plus whichever of `utm_source`/`utm_medium`/`utm_campaign` the landing page's own URL carried — captured once, from the browser, at conversation start, and never revisited (`Conversation.Source`/`TrafficSource.cs`). **Incidental PII, the same class `messages.body`'s own row above names**: a referrer or landing URL can occasionally carry a personalised path segment or an identifying query-string value that neither the widget nor this system parses for meaning — it is stored as opaque text, bounded to `TrafficSource.MaxLength` (512 characters) per field, the same length-cap discipline `messages.body` already applies to its own write path. It is also **unverified**: a client-supplied header, never confirmed against a second source — the console's own report copy says so. `null` for every conversation created before this shipped; no backfill. **`24-10`, added**: `blocked_at`/`blocked_by` - a tenant's own reversible suspension of processing for one conversation, set and cleared by an operator holding `Permission.ConversationBlock`, with each act and its reversal recorded separately in `conversation_block_records` rather than one row updated in place. A blocked conversation is hidden from every operator-facing read, is not routed by the assignment engine and is not auto-replied to - but an inbound message from the visitor is still **accepted and stored**, which is the whole distinction between blocking and erasure (`adr/0124`). **`23-69`/`23-77`, added**: `routing_suppressed_at` — stamped once, at `Conversation.Start`, only when the visitor already carried an active `visitor_restrictions` row (a time-windowed spam mute or an indefinite manual block, see that table's own row below) at the moment this conversation was created. Deliberately **not** `blocked_at`/`blocked_by` restated: nobody — no named operator — decided to freeze *this* conversation, so attributing it to one would misstate what happened; a second, honestly-named flag instead. A routing-suppressed conversation is kept from ever reaching an operator's queue or the automatic assignment engine the identical way a blocked one is (`WaitingConversationClaimQuery`, `GetOperatorQueueHandler`), and, like blocking, still **accepts and stores** the visitor's own messages — the silence both items' own dialogue with the author settled ("без всяких оповещений") is achieved by this conversation being ordinary in every way except routing, never by the widget being lied to | AGO | **Forever, and this stays adequate — the field rides the row's existing rule rather than needing one of its own.** Unlike `messages.body`, which earned a bespoke per-tier window (`adr/0031`/`13-06`) because it is this system's highest-volume, highest-sensitivity free-text store, these four columns are ordinary nullable columns on a row that already carries exactly one retention rule — forever, cascades from `sites` — for every field on it (the pairing, the timestamps, the counters). Four more nullable columns on the same row do not create a second retention surface to reason about; they are erased, exported and cascaded exactly when the rest of the row already is | Row deletion; cascades from `sites` | schema snapshot; `Stage18AddConversationTrafficSource.cs`; `Ago.Chat.Domain/TrafficSource.cs`; `Stage24AddConversationBlocking.cs`; `ConversationBlockRepository.cs` (`24-10`); `Stage23AddVisitorRestrictions.cs`; `Conversation.RoutingSuppressedAt` (`23-69`/`23-77`) |
| `visitor_restrictions` (`23-69`/`23-77`) | **A judgement about a person, and it is stored, per both items' own "Where this is likely to go wrong" sections.** One row per restricting act: which visitor, on which site, restricted by which operator, when, until when (or indefinitely), and — if reversed — when and by whom. Two triggers write it through the identical mechanism: `23-69`'s "close as spam" (`kind = Spam`, a real `expires_at` — a stated, configurable window, `ConversationSpamMuteOptions.DefaultDuration`, 24 hours by default, an engineering judgement call, not a measured number) and `23-77`'s manual "block visitor" (`kind = Block`, `expires_at = null`, reversed only by a deliberate act). `source_conversation_id` names the conversation the act was taken from — **deliberately no foreign key to `conversations`**, the identical `adr/0111`/`0112` "must survive the subject's own erasure" reasoning `access_records.resource_id` already states for itself one row above: a restriction naming the conversation that triggered it must remain nameable even after that conversation is later erased. **Enforced silently** — `StartConversationHandler` reads this table (`IVisitorRestrictionRepository.IsActiveAsync`) before creating a visitor's next conversation and, if active, stamps that new conversation's own `Conversation.RoutingSuppressedAt` (see the `conversations` row above) rather than refusing or altering the response — nothing here is ever shown to the visitor, and nothing about the wire response tells them they were checked at all | AGO, and the tenant through `GET /api/v1/visitor-restrictions` (`site:configure` — the same tenant-wide-oversight placement `access_records` above already uses, not a permission of its own, for the identical "a caller who can already read every conversation on the site gains nothing new to guard against" reasoning) | **Indefinite — the same "ordinary nullable columns on a row with one retention rule" shape the `conversations` row above states for itself**, not a timed audit log the way `access_records` is: a restriction is evidence of a judgement about a person that the tenant may need to revisit or defend, not routine staff activity safe to age out after a year. A naturally expired or manually lifted row is **not deleted** — `lifted_at`/`lifted_by` record the reversal in place, the same "current state and its own history live on the same row" shape this table's own append-only design already chose over one row updated in place | Row deletion; cascades from `sites` and from `visitors`. `16-02`'s conversation/visitor erasure reaches this table the same way it reaches `conversation_notes`/`visitor_contact_details` above — **not yet wired**, flagged rather than silently assumed: an erased visitor's own restriction history should plausibly go with them, and this item's own build did not reach that cascade | `Stage23AddVisitorRestrictions.cs`; `VisitorRestrictionRepository.cs`; `IVisitorRestrictionRepository.cs`; `CloseConversationAsSpamHandler.cs` (`23-69`); `BlockVisitorHandler.cs`/`LiftVisitorRestrictionHandler.cs` (`23-77`); `VisitorRestrictionRepositoryTests.cs` |
...
| `team_messages` (`23-32`, `removed_at` added `23-33`) | **A second kind of message store, this one deliberately outside the visitor conversation model.** Free text an operator typed to their own team - "I'm taking the angry one" - never visible to a visitor, never referenced from a `conversations`/`messages` row. Personal data about the *author*, not about a visitor: the same class `conversation_notes` above already establishes ("an operator's private annotation ... personal data about the visitor, written by someone else"), except here the subject and the author are the same operator. `author_is_admin` is not personal data itself - a boolean stamped from a permission check at send time (`SendTeamMessageHandler`'s own remarks, `ago-chat`), not a fact about the person beyond the role they held that moment. **`23-33`: a removed message's `body` is not scrubbed by this removal** - the account owner's own moderation hides it from every future read (`TeamMessageReadStore` nulls `body` whenever `removed_at` is set, before the row ever crosses into `Ago.Chat.Application`) but does not delete the text at rest, the same "row stays, content stops being served" shape a `Deleted` `Attachment`'s own row already takes. `removed_at` itself is a fact about the *message*, not a new fact about a person beyond what this row already held | AGO | **Forever, no retention window - the same "no per-tier class, no archive, no partition" shape `conversation_assignments` above takes, for an unrelated reason**: this table is small (bounded by how much a tenant's own team actually talks, not by visitor traffic) and internal, so none of `messages.body`'s per-tier retention machinery (`adr/0031`) exists for it, and none is planned unless real volume says otherwise (`TeamMessageConfiguration`'s own remarks: "CLAUDE.md: performance claims need numbers"). **Removal (`23-33`) does not shorten this** - a tombstoned row is retained exactly as long as one nobody ever moderated, only served differently | Row deletion; `ON DELETE CASCADE` from `sites` (a plain FK, the identical mechanism `operators`/`tags` above already ride, not `16-02`'s bounded-batch `ConversationErasureJob` - a team message has no attachment, no archive, nothing a bare cascade cannot clean up by itself). **No standalone "erase just my messages" path** - the same "the whole room goes, or none of it does" shape `operators`' own row states for a lone account; a team message is removed only by the site's own erasure, never independently. **`23-33`'s own removal is not an erasure path either** - it hides content, it does not delete the row or shorten its life | `Stage23AddTeamChat.cs`; `TeamMessage.cs`; `TeamMessageConfiguration.cs`; `SiteErasureQuery.cs` (its own `DeleteSiteAsync` doc comment names `team_messages` among the cascaded tables); `Stage23AddTeamMessageRemoval.cs`; `TeamMessageReadStoreTests.cs` |
| `team_message_removals` (`23-33`) | **Who removed a team message, and when - the accountability record the backlog item's own "a removal is itself a fact somebody may need to account for" calls for**, the same class `module_revoke_overrides` above already establishes for a different exceptional act: "the power exists, it is asymmetric, and exercising it leaves a record." `removed_by_operator_id` is personal data about the *remover* - an AGO Chat operator, the tenant's own account owner, not AGO's own staff (contrast `module_revoke_overrides.revoked_by`, which names a platform-owner subject). `team_message_id`/`site_id` name the message and the room, not a person | The tenant (via whichever operator holds `site:manage_operators` - `RemoveTeamMessageHandler`'s own remarks). Never leaves the deployment | **Indefinite while the room exists, no separate window of its own** - unlike `module_revoke_overrides`, this is not evidence of AGO acting against a tenant that might need to survive the tenant's own account closing; it is the tenant's own internal moderation log, about the tenant's own team, and lives exactly as long as `team_messages` does (this row's own "What removes it" column) | Row deletion; `ON DELETE CASCADE` from **both** `sites` and `team_messages` - **deliberately diverging from `module_revoke_overrides`' own no-FK choice**, and stated as a divergence rather than an oversight: that table's no-FK choice exists specifically so a record of AGO's own exceptional act survives the tenant's account closing, for a tenant who might later ask "who took this away from me"; this table answers a question asked *within* one tenant's own team, about content that belongs entirely to that team's own room, so `23-32`'s own "erasing the site erases the room" reaches it too - a removal record that outlived the room it describes would be an unreachable fragment of an operator's personal data with nothing left to attach it to. **Within ordinary operation** (the tenant still exists, one message among many was removed) **this record does survive the message it describes**, exactly as the backlog item's own Done-when asks: `team_messages` rows are tombstoned, never deleted independently of the site, so this FK only ever fires alongside that same site-wide cascade | `Stage23AddTeamMessageRemoval.cs`; `TeamMessageRemovalEntity.cs`; `TeamMessageRemovalEntityConfiguration.cs`; `adr/0133`; `TeamChatRepositoryTests.cs`; `SiteErasureQuery.cs` (updated `DeleteSiteAsync` doc comment) |
| `role_change_records` (`23-72`) | **Who changed a colleague's role, from what to what, and when - the accountability record the backlog item's own "every role change is recorded" calls for**, the same class `team_message_removals` above already establishes for a different exceptional act: "the power exists, it is asymmetric, and exercising it leaves a record." `changed_by_operator_id` and `changed_operator_id` are both personal data about AGO Chat operators - the tenant's own account holders, not AGO's own staff (contrast `module_revoke_overrides.revoked_by`, which names a platform-owner subject). `previous_role_names`/`new_role_name` name roles, not people | The tenant (via whichever operator holds `site:manage_operators` - `ChangeOperatorRoleHandler`'s own remarks). Never leaves the deployment | **Indefinite while the site exists, no separate window of its own** - `team_message_removals`' own reasoning, restated: this is not evidence of AGO acting against a tenant that might need to survive the tenant's own account closing, it is the tenant's own internal administration log, about the tenant's own team | Row deletion; `ON DELETE CASCADE` from `sites` and from `operators` (both `changed_by_operator_id` and `changed_operator_id`) - the same "ordinary tenant data, no reason to survive the tenant's own erasure" choice `team_message_removals` makes, for the identical reason: a role-change record that outlived the tenant it describes would be an unreachable fragment of an operator's personal data with nothing left to attach it to | `Stage23AddRoleChangeRecords.cs`; `RoleChangeRecordEntity.cs`; `RoleChangeRecordConfiguration.cs`; `RoleChangeRecordRepository.cs`; `ChangeOperatorRoleConcurrencyTests.cs` |
| `conversation_assignments` (`23-03`) | **Not personal data.** An operator id, a conversation id, two timestamps and a closed-vocabulary provenance label - no visitor identity, no content, nothing a person typed. Included here anyway because `decisions.md` §2 draws the distinction explicitly ("timestamps are not personal data, message content is") and because this table's own retention deliberately diverges from the conversation it names, which is exactly the kind of fact this register exists to make loud rather than discover at the first deletion | AGO | **Forever - independent of the conversation's own lifecycle.** Not "follows the owning conversation" the way `conversation_notes`/`conversation_tags` above do: those two hold personal data and must go when the conversation's does; this one holds none and is kept precisely so a tenant's workload numbers survive a visitor's own erasure request | **No foreign key on `conversation_id`, deliberately - not row deletion, and not `16-02`'s erasure.** `ConversationErasureQuery.DeleteConversationAsync` removes the whole `conversations` row on request; a cascading FK would take this table's rows with it, reversing `decisions.md` §2's own "erasing a conversation need not take last month's numbers with it." `site_id`/`operator_id` still cascade normally (from `sites`/`operators`), which only matters if a whole site or operator is ever hard-deleted - the demo-tenant sweep above is the one real case, and it is not a per-person erasure | `Stage23AddConversationAssignments.cs`; `ConversationAssignmentInterval.cs`; `ConversationAssignmentIntervalConfiguration.cs` |
| `acceptance_records` (`24-01`) | That a subject (a tenant, an operator, or a visitor - `adr/0076`'s own three-way split, named by `subject_kind` plus a bare `subject_id` rather than three nullable FK columns, `AcceptanceSubjectKind`'s own remarks) accepted a specific version of a specific document, at a specific instant - plus two narrow request-context fields, `client_ip?` and `user_agent?`, kept because a consent record is commonly indefensible without them and nothing wider: no referrer (`conversations.Source` already owns that), no session or correlation id, no device fingerprint, no copy of the document's own text (`ago-business`'s and a lawyer's, per this file's own "What is not decided here") | AGO. Never leaves the deployment - no integration event carries it, no webhook payload references it | **Indefinite - kept whole as evidence of a lawful basis at the time of processing, not a timed record about the subject.** Decided in `adr/0111`, the same shape of argument `visitor_contact_details` above already uses for its own indefinite retention: named, reasoned about, and stated here rather than a byproduct of nobody having built deletion. **Does not follow the erasure of the subject it names** - a real, deliberate exception, restated below | **Nothing today.** Deliberately: `adr/0111` decided erasure must not reach this table, so `subject_id` carries **no foreign key** to `sites`/`operators`/`visitors` - the same no-FK mechanism `conversation_assignments` above already uses, for a different reason (there: a workload count should outlive a visitor's own erasure; here: evidence should outlive any subject's own erasure). Neither `ConversationErasureQuery`'s per-conversation drain nor `SiteErasureQuery.DeleteSiteAsync`'s cascade reaches it, proven by `AcceptanceRecordErasureGuardTests` for both a visitor's own conversation erasure and a whole site's erasure - a genuine, accepted gap: a `subject_id` can outlive the row it once named, the identical trade-off `adr/0101` already accepted for `conversation_assignments` | `Stage24AddAcceptanceRecords.cs`; `AcceptanceRecord.cs`; `AcceptanceRecordConfiguration.cs`; `adr/0111`; `AcceptanceRecordErasureGuardTests.cs`. **Written at tenant registration since `24-03`** (`RegisterSiteHandler`, one row per key in `required_documents` for `Tenant`, in the registration's own transaction) - the first writer this table has; `24-01` built it with none, and `required_documents` is itself not in this register because it holds no data about a person |
| `erasure_records` (`24-13`) | **Metadata only, and the omissions are the design.** Scope (`Conversation`/`Site`), `site_id` (the tenant, not a person), `requested_by` (an operator id), status, `failure_reason` (an exception **type name**, never a message - a message can quote an object key), eight per-step counts, and timestamps. **No `conversation_id`, no `visitor_id`, anywhere on the row**; a conversation erased under a whole-site cascade gets no row of its own, only the site's aggregate `conversations_marked_for_erasure`. `adr/0112` argues why this points opposite to `acceptance_records` directly above it: an acceptance must name who consented, an erasure receipt that names the erased person keeps the pointer that was meant to disappear | AGO. Never leaves the deployment | **Indefinite, chosen rather than defaulted** - the same posture `export_requests` and `adr/0111` take, stated so a later reader does not mistake it for nobody having built pruning. **`Completed` means everything this process could reach, now - not gone from backups**: `adr/0050`'s thirty-day window on collected copies still applies, and a restore inside it can hold what a `Completed` receipt describes as erased | Row deletion only. **No cascade reaches it, deliberately**: neither `site_id` nor `requested_by` carries a foreign key, so a site-scoped erasure's own receipt survives the `DeleteSiteAsync` that erasure performs - `adr/0111`'s mechanism, reused for the opposite-facing reason | `Stage24AddErasureRecords.cs`; `ErasureRecordQuery.cs`; `ErasureRequestRepository.cs`; `adr/0112`; `ErasureRecordIntegrationTests.cs` |
| `access_records` (`24-12`) | **Who reached a boundary-crossing surface, when, and — where the access named one — which resource**: an operator's cross-conversation history read (`18-07`), or the platform owner's site list, site detail, module grant, module revoke or channel-identity unlink. **Itself personal data about the operator or platform owner who made the access** — `actor_id` is an `OperatorId` or a Keycloak subject. **Holds nothing about what was read**: no body, no preview, no copy of the row reached. Unlike `erasure_records` directly above, `resource_id` *does* name the conversation, channel identity or enabled module — `adr/0113` argues why the two point opposite ways one item apart: an erasure receipt naming its subject keeps the pointer that was meant to disappear, an access record not naming the resource answers a question nobody asked. **Refused attempts are not recorded** — a caller who read nothing has nothing to attest to | AGO, and the tenant named by `site_id`, through `GET /api/v1/sites/{siteId}/access-records` (`site:configure`, deliberately **not** a permission of its own: the same permission already gates `/admin`, so a caller who can reach this log already reads every conversation it describes, and a separate one would guard nothing from them) — which deliberately also shows AGO's own platform-owner accesses of that tenant's data, because hiding them would make unreadable the one question a tenant opens this table to ask | **365 days, chosen and enforced** by `AccessRecordPruneJob`. This diverges from `acceptance_records` and `erasure_records`, both indefinite, and the reason is the direction the data points: those are evidence about a subject's lawful basis or a completed erasure, this is evidence about **our own operators** making ordinary lawful reads, and keeping it forever would hoard staff data to answer a question nobody asks about year-old reads | The prune job's batched delete past the window. **No foreign key to `sites` or to the resource named, deliberately** — `adr/0111`/`0112`'s mechanism for a third reason: a record of an access to a tenant's data must survive that tenant's own erasure, or the tenant most likely to ask loses the answer to the very process they are asking about | `Stage24AddAccessRecords.cs`; `AccessRecordRepository.cs`; `AccessRecordPruneJob.cs`; `adr/0113`; `AccessRecordRepositoryTests.cs` |
| `contact_reveals` (`23-11`) | **Who revealed a visitor's contact detail, when, on which conversation, and from which surface — never the value revealed.** `operator_id` is personal data about the *operator*, the same category `access_records.actor_id` already is. Holds no copy of the phone number or email it is a record about, which is what keeps a log of unmaskings from becoming a second unmasked store | AGO, and the tenant, through `GET /api/v1/sites/{siteId}/contact-reveals` — `site:configure`, deliberately not a permission of its own, on `access_records`' own reasoning: an operator who already reads every conversation on the site, contacts included, is not additionally guarded by a separate permission over the log of who revealed what | **365 days, chosen and enforced** by `ContactRevealPruneJob` — `access_records`' window and `access_records`' reason: this is evidence about an ordinary, lawful, tenant-authorised read by our own operator, not proof of a lawful basis or of a completed erasure, so it does not earn `acceptance_records`' or `erasure_records`' indefinite retention | The prune job's batched delete past the window. **No foreign key on `site_id` or `contact_detail_id`** — the fourth use of `adr/0111`/`0112`/`0113`'s mechanism, here so a reveal record survives both `SiteErasureJob`'s cascade and `23-08`'s conversation-scoped contact-detail erasure | `Stage23AddContactVisibilityRung.cs`; `ContactRevealRepository.cs`; `ContactRevealPruneJob.cs`; `adr/0123`; `ContactRevealRepositoryTests.cs` |
| `channel_deliveries` (`23-19`, AGO Inbox) | **Whether one outbound channel message left, and to what — by reference, never by copy.** `site_id`, `conversation_id`, the triggering `message_id`, `channel_kind`, a reference to the `channel_identities` row it went through (`channel_identity_id`), the provider's own outcome, an optional provider message id and failure reason, and `attempted_at`. **No message body, and no second copy of the address** — `adr/0116` argues why that is a reference here where `adr/0112`/`0113` deliberately hold none | AGO, and the tenant, per conversation, gated the same way conversation reads are | **Its own window, enforced by `ChannelDeliveryPruneJob`** — an operational record, not evidence of a lawful basis, so unlike `acceptance_records` and `erasure_records` it is not kept indefinitely | The prune job's batched delete, plus `ON DELETE CASCADE` from `sites` and from `channel_identities` — a whole-site erasure removes it immediately rather than on the window. **`conversation_id` carries no foreign key, deliberately**, so the record survives a single conversation's own erasure | `Stage23AddChannelDeliveries.cs`; `ChannelDeliveryConfiguration.cs`; `ChannelDeliveryPruneJob.cs`; `adr/0116` |
| `module_revoke_overrides` (`23-13`) | **Who overrode a tenant's own purchase, when, which site, which module, and why, in free text.** `revoked_by` is the platform owner's Keycloak `sub` — personal data about AGO's own staff, the same category `access_records.actor_id` already is. `reason` is deliberately free text, unlike `erasure_records.failure_reason`'s exception-type-name discipline: the whole point of this row is that the override must be *justified*, and a justification that cannot name what happened is not one (`decisions.md` §6) — so it may incidentally name a person the tenant's own customer is, the same "free text a person typed" caveat `conversation_notes` already carries. `site_id` names the tenant, not a person, and `module_key` is a snapshot of a key that no longer resolves to a live row by the time this is written | AGO. Never leaves the deployment | **Indefinite, chosen rather than defaulted** — the same posture `erasure_records`/`export_requests` take, and deliberately not `access_records`' 365-day window: this is evidence of an exceptional act taken *against* a tenant, not an ordinary lawful read, and the item that created it argues explicitly that the override "is the act which later has to be justified, possibly to the person it was used against" — a claim a pruned row could not support | Row deletion only. **No foreign key to `sites`, deliberately — the fourth instance of `adr/0111`/`adr/0112`/`adr/0113`'s own mechanism**: a tenant whose purchase was overridden and who later closes their account is exactly the tenant most likely to ask, later, who did this and why, and a cascade would erase the answer with the account | `Stage23AddModuleRevokeOverrides.cs`; `ModuleRevokeOverrideRepository.cs`; `adr/0118`; `RevokeModuleForSiteAsOwnerHandlerTests.cs`; `ModuleRevokeOverrideRepositoryTests.cs` |
| Minted demo tenants (`8-07`) | Whatever a viewer typed into their own demo, in the same tables as any other tenant - plus a Keycloak user with a synthetic `@demo.invalid` address (RFC 2606, unroutable) that nothing sends to | AGO | **About a day**, then removed automatically - the only data in this system with a bounded life that is actually enforced | `DemoTenantExpiryJob`: MinIO objects, then `DELETE FROM sites` (cascading to every table above) **together with a `SiteErased` outbox row, staged in the identical transaction** (`25-82`, rule 4), then the Keycloak user. That row is what lets `role_assignment_projections` (**AGO Calendar**, this table's own row below) stop accumulating for a tenant this job removed - `Ago.Calendar.Worker.SiteErasedConsumer` drains it once the outbox dispatcher delivers the event, reached asynchronously the same way every other outbox-fed cross-product fact in this map already is. Found live, 2026-09-14: 70 of 74 distinct `tenant_id`s that table held belonged to demo tenants this job had already deleted, with nothing ever telling AGO Calendar so. **Still does not reach**: backups until `15-02`'s window, node queues, traces, logs, Redis entries, or message-archive objects in object storage - the `message_archives` row cascades with the site, but the archived `.zip` stays orphaned in storage, the same MinIO-object-outlives-its-row gap the attachments row above already names - `adr/0058` has the full table | `adr/0058`; `DemoTenantExpiryJob`; `DemoTenantLifecycleTests`; `SiteErasurePublisher.cs`; `SiteErasedOutboxTests.cs` (`25-82`) |
...
| `operators` | `id`, `site_id`, `status`, `capacity`, `active_chats`, `external_subject_id`, and — **since `23-02`** — `display_name`/`email`. **These two are a copy, not a join**: captured from the validated token's own `name`/`email` claims at invite redemption (`OperatorInviteRedemptionRepository`) or bootstrap registration (`RegisterSiteHandler`), and **rewritten at every sign-in** (`GetMyPermissionsHandler`, the handler behind `GET /api/v1/operators/me`, which the console calls once per session) rather than queried live from Keycloak — the alternative `docs/design/decisions.md` §1 rejected explicitly, because a synchronous per-request dependency from `ago-chat` to the IdP is a new failure mode this product does not have today. The refresh is a conditional `UPDATE … WHERE display_name IS DISTINCT FROM @name OR email IS DISTINCT FROM @email`, so an ordinary session that changes nothing about the caller's identity costs one statement and writes no row (`OperatorRepository.RefreshIdentityAsync`, proven by `OperatorIdentityRefreshTests`, `ago-chat`). **`MintDemoTenantHandler`'s minted operators carry neither** — that identity is never authenticated through Keycloak, so there are no claims to copy, and none are invented | AGO | Forever | Row deletion; `ON DELETE CASCADE` from `sites` (`Stage1CreateChatSchema`'s own `FK_operators_sites_site_id`) — the same cascade this row already had, unwidened by these two columns. **`16-02`'s own site-erasure job reaches it already, verified rather than assumed**: `SiteErasureQuery.DeleteSiteAsync`'s own remarks list `operators` among the tables its final `delete from sites` cascades into for free, and that was true before this item and stays true after it — two more nullable columns on an already-cascading row add nothing for that job to do differently. **No standalone "erase just my name" path exists** — the same "the whole row goes, or none of it does" shape every other field on this row already has; a lone operator's own account is removed only by the site (or the row) going with it, not by field | schema snapshot; `authorization.md`; `docs/design/decisions.md` §1; `SiteErasureQuery.cs` |
...
| `customers` (**AGO Calendar**'s own Postgres, `20-01`) | **A phone number — the customer's only mandatory field — plus an optional name, free-text operator notes, and a no-show count.** The most directly identifying store either product has: unlike `visitors`, this row is *meant* to name a person, and unlike `messages.body` the identifier is structured and exact. **`23-60`/`adr/0161` adds `merged_into_customer_id`/`merged_at`**: a row an operator merged away is tombstoned, not deleted (`Event.CustomerId`'s own foreign key and this table's own "kept forever" rule are why) - it stays exactly the personal data it always was, just no longer shown on the ordinary contacts screen | AGO, on its own node | **Forever.** No pruning exists, and none is designed. A tombstoned row's retention does not shorten - it follows the same tenant-wide cascade as every other row here | **`22-30`: reached by AGO Chat's `SiteErasureJob` module gate, over `DELETE /api/v1/module-registrations/{tenantId}/tenant-data`** (authenticated by the deployment-wide provisioning secret, `adr/0149` rule 2 - "the module proves it") - `TenantErasureRepository.EraseAsync` deletes the `Tenant` row, `ON DELETE CASCADE` takes this table with it, and a fresh read of Postgres confirms nothing remains before AGO Chat's own site row is allowed to go. No longer "nothing automatic". The self-referencing `merged_into_customer_id` foreign key cascades within the same delete, tombstoned rows included | `Stage20CreateCalendarSchema.cs`; `Ago.Calendar.Domain/Customer.cs`; `TenantErasureRepository.cs`; `SiteErasureJob.EraseModulesAsync`; `Stage23AddCustomerMergesAndTombstone.cs` (`23-60`) |
| `customer_merges` (**AGO Calendar**, `23-60`/`adr/0161`) | **Who merged which of a tenant's own customer records into which, when, and how many bookings moved** - the accountability record `adr/0147`'s own "a merge that cannot be explained afterwards is a merge nobody will trust" calls for, the same class `role_change_records`/`team_message_removals` already establish for a different exceptional act: "the power exists, it is asymmetric, and exercising it leaves a record." `operator_id` is personal data about the *operator*, the same category those two tables' own equivalent columns are. `survivor_customer_id`/`absorbed_customer_id` name two of the tenant's own customers, not a third party | The tenant (via whichever operator holds `customer:edit` - `MergeCustomersHandler`'s own remarks). Never leaves the deployment | **Indefinite while the tenant exists, no separate window of its own** - `role_change_records`' own reasoning, restated: this is not evidence AGO itself might need to answer for after the tenant is gone, it is the tenant's own internal record of its own act on its own data | Row deletion; `ON DELETE CASCADE` from **all three** of `tenants`, and the `customers` rows named by `survivor_customer_id`/`absorbed_customer_id` - **deliberately diverging from `contact_phone_reveals`' own no-FK choice** one row above, and stated as a divergence rather than an oversight: that table's no-FK choice exists so evidence of AGO's own operators' reads survives a tenant's own erasure; this table answers a question asked *within* one tenant, about that tenant's own customers, so the ordinary tenant-erasure cascade reaching it is correct, not a gap | `Stage23AddCustomerMergesAndTombstone.cs`; `Ago.Calendar.Application.Abstractions/ICustomerMergeStore.cs`; `ICustomerMergeReadStore.cs`; `adr/0161` |
| `pending_phone_verifications` (**AGO Calendar**, `20-10`) | **A raw phone number in the clear**, plus a hashed code, a hashed bearer proof token, and their own separate expiries. The one place in either product's own Postgres that holds a phone number without an accompanying name or booking history — this table exists purely to prove a stranger's browser controls a number, before any lead card is ever created. Written by the public, unauthenticated booking widget's own verification flow (`20-10`), independent of `ago-chat`'s identically-shaped `pending_phone_verifications` (`14-15`) - different database, different repository, `adr/0027` | AGO, on its own node | **Forever today — no pruning exists.** A confirmed-and-booked row's code/proof are already spent (hashed, single-use, short-lived by their own `expires_at`/`proof_expires_at`), so the residual risk is a stale row's own phone number, not a replayable secret; still a real gap the same shape `ago-chat`'s own `14-15` table has, not yet closed for either product | **`22-30`: reached the same way `customers` above now is** - `ON DELETE CASCADE` from `tenants`, inside the same tenant erasure. No longer "nothing automatic" | `Stage20AddPendingPhoneVerifications.cs`; `Ago.Calendar.Domain/PendingPhoneVerification.cs`; `TenantErasureRepository.cs` |
| `events` (**AGO Calendar**) | Not content, but a **behavioural record about a named person**: which worker they saw, for what service, when, and whether they turned up. `customer_id` is retained on cancelled and no-show rows deliberately, because the lead card exists to keep exactly that history | AGO | Forever | **`22-30`: reached the same way `customers` above now is** - `ON DELETE CASCADE` from `tenants`, inside the same tenant erasure, bypassing the ordinary per-row deactivation rules a live `Event`'s own aggregate might otherwise apply (a whole-tenant erasure is a stronger act than any single booking's own lifecycle) | `Stage20CreateCalendarSchema.cs`; `Ago.Calendar.Domain/Event.cs`; `TenantErasureRepository.cs` |
| `role_assignment_projections` (**AGO Calendar**, `22-05`) | `operator_id`, `tenant_id`, **`external_subject_id`**, `permissions`, `updated_at`. **A Keycloak subject id is personal data**: it singles out one person and is the identifier that links this product's rows to a human being at the identity provider. **This row replaces the `operators` row that stood here until 2026-09-08 and described a table that no longer exists.** `22-05` dropped AGO Calendar's own `operators` and `roles` in favour of this projection, replicated from AGO Chat's outbox (`adr/0093`) - and the old row also called that table *cascaded*, which was true of `operators` and is **not** true of this one. The stale sentence had a cost: `22-30` found that a tenant erasure trusting that cascade would have left every operator's subject id behind for ever | AGO, on its own node | **No TTL of its own - removed only when the tenant it names is erased, and until `25-82` that only ever happened for a tenant erased through `22-30`'s own path.** A demo tenant (`8-07`) never goes through that path (`DemoTenantExpiryJob` is a scheduled hard delete, not an owner-initiated erasure request), so its own row here had genuinely no remover at all before `25-82` - not a long TTL, an *absent* one. Measured live, 2026-09-14: 70 of the 74 distinct `tenant_id`s this table held belonged to demo tenants already gone from `ago_chat.sites`. Whether that accumulation was itself a personal-data question or an ordinary staleness one was this item's own open question, left to the author rather than assumed either way (`docs/backlog/25-82-*.md`) | **Two paths now land on the identical `TenantErasureRepository.EraseAsync`, both deleting this table explicitly rather than by cascade** - it has **no foreign key to `tenants`**, deliberately, the same as every outbox-fed projection here, since a snapshot an idempotent consumer can replay is not an owned child of the aggregate it happens to be keyed by. **`22-30`**: synchronous, over HTTP, when an owner-initiated erasure reaches this product's own module-registration endpoint. **`25-82`, new**: asynchronous, `Ago.Calendar.Worker.SiteErasedConsumer` reacting to `ago-chat`'s own `SiteErased` outbox event - published only when `DemoTenantExpiryJob`'s own `DELETE FROM sites` actually removes a row, in the same transaction (rule 4) - which is what reaches a demo tenant the first path never did, since a demo tenant never registers this product's module and so never has anything for `22-30`'s own endpoint to be called about | `Stage22ReplaceOperatorsWithRoleAssignmentProjections.cs`; `TenantErasureRepository.cs`; `RoleAssignmentProjectionRecord.cs`; `SiteErasedConsumer.cs`; `SiteErasedConsumerDemonstrationTests.cs` (`25-82`) |
| `chat_booking_tasks` (**AGO Calendar**, `20-07`) | **A raw phone number in the clear**, plus the calendar, service, worker and event a chat-originated booking is for. **Added to this map on 2026-09-08, having been absent since the table existed** - found by checking the map against the live schema rather than against itself, which is the check `23-101` exists to make routine. It is the third place in this product holding an unhashed phone (`customers` and `pending_phone_verifications` are the others), and unlike those two it was never described here | AGO, on its own node | **Forever today - no pruning exists**, and a completed task's phone number is as durable as a live one's. The same gap `pending_phone_verifications` above records, and it should be closed for both or for neither | **`22-30`: `ON DELETE CASCADE` from `tenants`**, inside the tenant erasure - this table does carry that foreign key, unlike the two projections above | `Stage20AddChatBookingTasks.cs`; `Ago.Calendar.Domain/ChatBookingTask.cs`; `TenantErasureRepository.cs` |
| `workers` (**AGO Calendar**, `20-13`) | **Personal data about the tenant's own staff, not a customer's.** `last_name`/`first_name` (required), `middle_name` (optional) — a real name, unlike the bare `display_name` this row held before `20-13`. `display_name` itself is what the public booking surface and every console list actually render; the three name fields never leave the console, per the author's own instruction when `20-13` was scoped ("Показываем 'отображаемое имя'") | AGO | Forever. No pruning exists | Row deletion; `ON DELETE CASCADE` from `tenants`. **`20-13`'s own deletion endpoint refuses to remove a worker with any booking history** (`PendingConfirmation`/`Booked`/`NoShow`), so in practice a worker who was ever booked is deactivated, not erased — the same shape `Customer` rows already have for the same reason (the lead card's own history). **`22-30`: a whole-tenant erasure bypasses that per-row refusal** — the cascade from `tenants` removes every worker regardless of booking history, because the tenant itself is going, not one worker within it | `Stage20AddWorkerNameFieldsAndTimestamps.cs`; `Ago.Calendar.Domain/Worker.cs`; `TenantErasureRepository.cs` |
...
| **The visitor's own browser** — `localStorage`, keys `ago-chat:<siteKey>:*` (`24-15`) | **A `visitor-id` and a signed `visitor-token`**, plus a cached copy of the tenant's own widget settings (colour, position, locale, notice text, notice URL) and the `conversation-id` this browser last held. The identifier is the interesting half: it is what makes a returning visitor recognisable across visits, and it is **renewed rather than re-minted** (`17-07`) precisely so one person does not fragment into many. **No cookies at all** — the widget sets none, and the transport default that would have sent the host page's cookie jar is explicitly disabled (`connection.ts`), so a tenant auditing their own site for cookies finds nothing and would wrongly conclude there is nothing to declare | **The visitor, and only the visitor.** Not AGO and not the tenant: it is on somebody else's device, behind an origin neither of us controls. The keys are prefixed per site, so the widget can neither collide with the host page's own storage nor read a key it did not write | **Nothing expires the identifier.** The token is issued for 7 days and renewed for as long as the visitor keeps returning, so a returning visitor's stored record effectively never lapses; the cached tenant settings are refreshed on each renewal. Stated as the honest answer rather than left to imply a window that does not exist | **Nothing we can do.** This is the one row in this register where AGO has no erasure path at all — we cannot reach the store, so `16-02`'s erasure does not touch it and no future job can. What clears an entry is the visitor clearing their own browser's site data, or the specific replacement rules in each key's own Lifetime. The register says so rather than implying a reach we do not have | `ago-widget/src/storage.ts` (`WIDGET_STORAGE_DISCLOSURE`); `ago-widget/src/connection.ts`; the tenant-facing screen at `/settings/device-storage` (`ago-console`), whose rows are asserted against that constant so the document cannot drift from the code |
...
| Tenant export archive (MinIO, `16-03`) | The same stores this table lists for a site, as of the moment the export ran — conversations, messages, attachments (referenced by presigned URL, not embedded — `adr/0072`), operators, visitors, channel identities, site configuration. **Since `22-31`**, also the calendar add-on's own data — reached over the existing deployment-wide provisioning channel (the same one `22-30`'s erasure already uses) and embedded **opaquely**, as one member chat never parses (`adr/0149` rule 3), listed by key/format-version/byte-count in `manifest.json`. A full copy, one file per tenant per export, at `exports/site/{siteId}/{exportId}.zip` | AGO, on its own node (chat's own half); the calendar's own bytes are AGO-held too, just embedded rather than reachable in their own right — a new place calendar-held personal data exists outside the calendar's own database | **Forever.** No expiry job exists — only the *embedded attachment links* inside it decay, after 7 days (`adr/0072`); the archive object itself is not pruned. `22-31` changes nothing here: the calendar's own member shares the archive's one lifetime, not a second one | Nothing automatic today. Not yet reachable by `16-02`'s erasure (see below) | `SiteExportJob.cs`; `SiteExportArchiveWriter.cs`; `adr/0072`; `Ago.Chat.Application.Abstractions.IModuleRegistrationGateway.ExportTenantDataAsync`; `Ago.Calendar.Infrastructure.Postgres.TenantDataExportWriter` (`22-31`) |
...
### `23-60`/`adr/0161` lets a tenant merge two customer rows into one, and records that it did

**A merged-away row is tombstoned, not deleted.** `customers.merged_into_customer_id`/`merged_at`
are the two new columns; every field this file already inventoried for `customers` (phone, name,
notes, no-show count) stays exactly where it was on that row - a merge changes which record an
operator is pointed at going forward, it does not erase what the losing record held. `Event.CustomerId`'s
own foreign key is why: deleting the row outright would either orphan a booking's own history or force
a second, un-audited reassignment this decision would rather make visible.

**`customer_merges` is personal data about the operator who merged, the same class
`contact_phone_reveals` above already establishes for a different act** - see this file's own new
table row for the retention and erasure story, which deliberately diverges from that row's: a merge
record cascades with the tenant, a reveal record does not, and `adr/0161` states why.

**No e-mail field exists on `customers` to widen this file's own inventory with.** `23-60`'s own Scope
line names "a phone or an e-mail"; this product's `Customer` aggregate has never held one, on any row,
so nothing new is collected here - `adr/0161` states this as a scope boundary rather than a silent gap.
...
**Two clocks, in the same table, and `23-08` is what keeps them from being read as one.** `messages.body`'s
own row runs on a timer — per tier, per `adr/0031`/`13-06`, archived once the window closes.
`visitor_contact_details`, three rows below it, runs on no timer at all: it ends when the person asks
for erasure or the tenant judges it useless, and otherwise stands **indefinitely**, because it is the
tenant's asset rather than a record kept about the visitor (`docs/design/decisions.md` §4, correcting
the ninety days first proposed for it). The two are linked, not cascaded: a person's own erasure request
takes both, because it is all their data; a retention sweep of aged-out messages takes only the
transcript, or the tenant would lose an asset every time one aged out.

**A third clock, added `24-01`, and it runs backwards from the other two on purpose.**
`messages.body` and `visitor_contact_details` both eventually answer to a person's own erasure request
— one on a timer, one on the person asking. `acceptance_records` (`adr/0111`) deliberately does **not**:
it is kept whole, indefinitely, as AGO's own evidence that a subject's processing had a lawful basis at
the time, and neither `16-02`'s conversation-scoped nor site-scoped erasure reaches it — proven, not
merely asserted, by `AcceptanceRecordErasureGuardTests`. Naming it a "clock" at all is almost a
misnomer: unlike the other two, nothing is meant to start it running. If that turns out to be wrong —
if a future legal reading requires an acceptance record to be erasable after all — `adr/0111` is
superseded, not quietly patched, the same posture `adr/0076` already committed this file to for the
controller/processor split this decision depends on.
...
merely fail to notice one.

**`operators` gained a name and an email in `23-02`, and this paragraph used to warn against exactly
that** — "not a small change: it converts erasure from a two-place problem into a five-place one." The
warning was honoured by making the change match its own premise rather than by skipping the change:
`display_name`/`email` were added to the *existing* `operators` row and nowhere else. No read model
stores a copy — `OperatorAnalyticsReadStore`, `ConversionReportReadStore` and `ConversationReadStore`'s
own admin list all `JOIN operators` at read time and hand back a name that is never itself written to
disk a second time. No integration event carries one either. So the number of places a name can be
found in this system is **one** — the `operators` row itself, exactly the "two-place" baseline this
paragraph originally measured against, not the five-place one it warned a careless version of this
...
- `.claude/skills/embeddable-widget` — **added 2026-09-06 by `24-15`**, on adding anything to the
...
Its eight gaps were `24-07`…`24-14`; `24-11`, `24-12`, `24-13` and `24-14` have since closed, leaving `24-07`, `24-08`, `24-09` and `24-10`.
