...
| `role_assignment_projections` (**AGO Calendar**, `22-05`) | `operator_id`, `tenant_id`, **`external_subject_id`**, `permissions`, `updated_at`. **A Keycloak subject id is personal data**: it singles out one person and is the identifier that links this product's rows to a human being at the identity provider. **This row replaces the `operators` row that stood here until 2026-09-08 and described a table that no longer exists.** `22-05` dropped AGO Calendar's own `operators` and `roles` in favour of this projection, replicated from AGO Chat's outbox (`adr/0093`) - and the old row also called that table *cascaded*, which was true of `operators` and is **not** true of this one. The stale sentence had a cost: `22-30` found that a tenant erasure trusting that cascade would have left every operator's subject id behind for ever | AGO, on its own node | Forever. No pruning exists | **`22-30`: deleted explicitly, not by cascade.** This table has **no foreign key to `tenants`** - deliberately, the same as every outbox-fed projection here, since a snapshot an idempotent consumer can replay is not an owned child of the aggregate it happens to be keyed by. `TenantErasureRepository.EraseAsync` issues its own `DELETE` for it inside the erasure transaction, and the confirming read afterwards names this table explicitly | `Stage22ReplaceOperatorsWithRoleAssignmentProjections.cs`; `TenantErasureRepository.cs`; `RoleAssignmentProjectionRecord.cs` |
