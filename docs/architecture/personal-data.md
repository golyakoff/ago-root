...
| `team_messages` (`23-32`) | **A second kind of message store, this one deliberately outside the visitor conversation model.** Free text an operator typed to their own team - "I'm taking the angry one" - never visible to a visitor, never referenced from a `conversations`/`messages` row. Personal data about the *author*, not about a visitor: the same class `conversation_notes` above already establishes ("an operator's private annotation ... personal data about the visitor, written by someone else"), except here the subject and the author are the same operator. `author_is_admin` is not personal data itself - a boolean stamped from a permission check at send time (`SendTeamMessageHandler`'s own remarks, `ago-chat`), not a fact about the person beyond the role they held that moment | AGO | **Forever, no retention window - the same "no per-tier class, no archive, no partition" shape `conversation_assignments` above takes, for an unrelated reason**: this table is small (bounded by how much a tenant's own team actually talks, not by visitor traffic) and internal, so none of `messages.body`'s per-tier retention machinery (`adr/0031`) exists for it, and none is planned unless real volume says otherwise (`TeamMessageConfiguration`'s own remarks: "CLAUDE.md: performance claims need numbers") | Row deletion; `ON DELETE CASCADE` from `sites` (a plain FK, the identical mechanism `operators`/`tags` above already ride, not `16-02`'s bounded-batch `ConversationErasureJob` - a team message has no attachment, no archive, nothing a bare cascade cannot clean up by itself). **No standalone "erase just my messages" path** - the same "the whole room goes, or none of it does" shape `operators`' own row states for a lone account; a team message is removed only by the site's own erasure, never independently | `Stage23AddTeamChat.cs`; `TeamMessage.cs`; `TeamMessageConfiguration.cs`; `SiteErasureQuery.cs` (its own `DeleteSiteAsync` doc comment names `team_messages` among the cascaded tables) |
