...
| `customers` (**AGO Calendar**'s own Postgres, `20-01`) | **A phone number — the customer's only mandatory field — plus an optional name, free-text operator notes, and a no-show count.** The most directly identifying store either product has: unlike `visitors`, this row is *meant* to name a person, and unlike `messages.body` the identifier is structured and exact | AGO, on its own node | **Forever.** No pruning exists, and none is designed | **`22-30`: reached by AGO Chat's `SiteErasureJob` module gate, over `DELETE /api/v1/module-registrations/{tenantId}/tenant-data`** (authenticated by the deployment-wide provisioning secret, `adr/0149` rule 2 - "the module proves it") - `TenantErasureRepository.EraseAsync` deletes the `Tenant` row, `ON DELETE CASCADE` takes this table with it, and a fresh read of Postgres confirms nothing remains before AGO Chat's own site row is allowed to go. No longer "nothing automatic" | `Stage20CreateCalendarSchema.cs`; `Ago.Calendar.Domain/Customer.cs`; `TenantErasureRepository.cs`; `SiteErasureJob.EraseModulesAsync` |
