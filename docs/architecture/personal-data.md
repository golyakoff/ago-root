...
| Redis rate-limit buckets (**AGO Calendar**, `20-03`) | **A SHA-256 of `tenant_id:phone`**, as the key of a token bucket — one per phone number that has attempted a booking. Written by the unauthenticated booking endpoint, which is the only public write surface either product has. The value holds a token count and a timestamp, nothing about the person. **Pseudonymised, not anonymised, and the distinction is the point**: the input space of phone numbers is small enough to enumerate, so a hash is still personal data — what it buys is that the number is not readable by eye in `KEYS` output, a slow-log line or a memory dump, and that two tenants' buckets for one person are unlinkable to a reader of the key space. The number itself is **never** written in clear, which `ThePhoneNeverAppearsInARateLimitKey` holds as an assertion | AGO, on its own node. Redis is **not** in the backup set (`adr/0050`), so no copy of this leaves the node | **Minutes.** The token bucket sets its own TTL, `ceil(capacity / refill_per_second) + 1` seconds — with `20-03`'s defaults that is about an hour for the phone bucket and a minute for the calendar one. The shortest-lived personal data in either product, and the only one that expires without anybody building a job | The key's own TTL; a Redis restart, since nothing here is persisted | `BookEventHandler.PhoneBucket`; `Ago.Platform.Caching.Redis/RedisRateLimiter.cs`'s Lua script |
