...
| `RUSTORE_PUSH_SERVICE_TOKEN` | `26-04`/`adr/0180`: RuStore Push's own long-lived bearer token - presented directly as `Authorization: Bearer` on every operator-push send, no OAuth2 mint, no second host | `.env` on the deploying machine → Secret, bound to `Push:RuStore:ServiceToken` | `Ago.Chat.Worker` **only** - never `Ago.Chat.Api`, never `Ago.Chat.Webhooks`, the identical isolation `KEYCLOAK_DEMO_PROVISIONER_SECRET`'s own row states for a different credential | Restart - RuStore's own documentation does not say whether a push project may hold two live service tokens at once, so the weaker class is recorded rather than the Draining a Google service account would have allowed for free (`adr/0180` §2) |
| `FCM_SERVICE_ACCOUNT_JSON` | `26-100`/`adr/0181`: the Firebase service-account key (client_email + RSA private key) FCM HTTP v1 requires; `Ago.Chat.Worker` mints a short-lived OAuth2 access token from it per send. FCM is the primary operator-push transport, RuStore the per-device fallback. `Ago.Chat.Worker`'s `FcmOptions` **validates it on start**, so the Worker will not boot without it once this code is deployed | `.env` on the deploying machine → Secret, bound to `Push:Fcm:ServiceAccountJson` (`Push__Fcm__ServiceAccountJson`) | `Ago.Chat.Worker` **only** - never `Ago.Chat.Api`, never `Ago.Chat.Webhooks` (`adr/0181` §4) | Draining - a Google service account may hold multiple active keys at once, so a new key can be added and traffic moved before the old one is deleted, unlike RuStore's Restart above |
