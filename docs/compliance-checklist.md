...
| H4 | A tenant can point at something and refuse a processing purpose we added | **built (`25-04`, 2026-09-14) — with one half still owed to the lawyer.** The AI features are no longer AGO's deployment-wide switch. They are a paid module, off for every tenant until that tenant buys it, accepts `ai-processing-addendum` at its current version (`24-01` record naming that version) and **separately declares** it holds a lawful basis for its visitors' data reaching the provider — a distinct, separately-timestamped, separately-attributed record (`ai_processing_basis_declarations`), because by art. 6 ч. 3 the basis must exist **for the visitor** and by ч. 4 obtaining it is the **tenant's** obligation, not ours. Enabling stamps a cut-off; nothing created before it is ever sent, proven against the background categoriser itself and not only against a handler. Point at: `AiProcessingGate`, `NothingReachesTheVendorWhenDisabledTests`, `ConversationCategorizationJobTests.RunOnceAsync_WithAConversationClosedBeforeTheCutOff_*`, and the console screen at `/account/ai`. **Still owed: the agreement text itself is a draft AGO wrote and a lawyer has not read** — `25-04`'s own Open questions (is point 5 enough as a form of declaration; must the provider be named in the text rather than in the console; what may point 6 promise about a retention `personal-data.md` records as *not established*). The mechanism is closed; the wording is not | mechanism built; **wording: lawyer** |
