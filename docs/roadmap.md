...
| 2 | `8-08` the schema migrator as a deployable | A missing migration fails no probe and no deploy: on 2026-08-25 the API served 200s on every page while every `Site` query failed, three migrations behind. Second because it gets harder with every migration added after it, and because the expand/contract discipline it forces is cheaper to start now than to retrofit (`adr/0056`) |
