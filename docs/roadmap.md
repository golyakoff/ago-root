...
| # | Item | Why here |
|---|---|---|
| 1 | `13-07` one login, several tenants | Priority (author, 2026-08-27): needed for today's demo. The `10-02` Account-aggregate question, revisited now that a real caller and a real answer (no shared billing) both exist |
| 2 | `14-02` MAX channel adapter | The first concrete channel, chosen because it has an open Bot API and no regulatory friction |
| 3 | `13-01` operator invitations and seat entitlement | Also carries seat assignment and operator removal, which `decisions/0006` made a prerequisite of everything else in billing. Read `13-07` first — it changes this item's own redemption check |
