...
| # | Item | Why here |
|---|---|---|
| 1 | `5-18` the console never connects to the operator hub | The live demo's headline promise - answer from the console - does not work: no operator ever holds a connection, so nothing is ever assigned and every conversation sits in Waiting. Found by walking the demo as a stranger. `smoke.sh` is green throughout, which is the other half of the item |
| 2 | `8-11` the demo pages still say there is one shared tenant | `8-06` wrote the honesty copy when that was true; `8-07` and `8-09` falsified it. On a minted tenant the widget still tells the visitor anyone can read their conversation, one click after the panel promised nobody can - a contradiction on screen, not staleness |
...
| 4 | `8-10` the migrator waits for its database | Found by the first deploy carrying `8-08`: Postgres was restarting, the Job lost the race, and `backoffLimit: 0` left it Failed until somebody re-ran it. The guard behaved correctly and the outcome was safe - it just needed a person. Not a migration failure; the migration never started, and the design cannot tell those apart |
