...
| 2 | `20-11` additional verified contact channels for a booking | Deferred from `20-09` under real time pressure, named as its own item rather than left a silent gap - priority-ordered extra channels (another phone, a messenger), each independently verified. `20-10`'s own mechanism now exists to build against |
| 3 | `20-19` third-party integration against the public booking API | `20-10`'s own gate turned out to have no real caller in `ago-widget` at all - `20-07` deleted that repository's entire direct HTTP client to Calendar five days before `20-10` was scoped against it. The endpoint's real caller is a tenant's own custom integration, decided 2026-09-01; this item is what makes that usable by someone outside this codebase |
