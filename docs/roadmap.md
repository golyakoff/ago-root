...
| 3 | `20-02` availability materialisation | Materialised ahead to a rolling horizon, with already-materialised days directly editable |
| 4 | `20-03` booking and the lead card | An atomic compare-and-set claim on an `Available` row — the same discipline operator capacity already uses |
| 5 | `20-04` confirmation sweep and operator queue | The same Worker-job shape as `OutboxDispatcher`, not a new mechanism |
| 6 | `20-06` Calendar console and booking widget | Reuses per-site CORS and the OIDC pattern rather than inventing a second of either |
| 7 | `14-01` external channel identity and inbound port | AGO Inbox starts here: the domain concept and the port every adapter plugs into |
| 8 | `14-04` offline auto-reply | Channel-agnostic by design, so it needs no adapter and no vendor — the one Inbox item with no external dependency |
| 9 | `14-02` MAX channel adapter | The first concrete channel, chosen because it has an open Bot API and no regulatory friction |
| 10 | `13-01` operator invitations and seat entitlement | Also carries seat assignment and operator removal, which `decisions/0006` made a prerequisite of everything else in billing |
| 11 | `13-02` ЮKassa checkout and webhook | The first successful payment; state confirmed by webhook through the outbox, never by trusting a redirect |
| 12 | `13-03` subscription lifecycle | Unblocked 2026-08-25 — renewal, failure, cancellation and mid-cycle changes are all decided policy now |
| 13 | `13-04` console billing view | Tier, seats used, upgrade and downgrade, out of `11-05`'s components |
| 14 | `15-04` retention and pruning jobs | Bounded pruning of everything that grows without limit, and the mechanism `13-06` builds on |
| 15 | `13-06` retention class and archive | The mechanism behind `adr/0031`: partition by immutable class, archive before dropping. Directly after `15-04` because it reuses that item's bounded-batch deletes and partition drops rather than writing a second copy |
| 16 | `15-05` capacity and disk | Deliberately not first any more: its prerequisite is done and the slope it needs is now accumulating, so the useful next step is to *let it run* and read the rate |
| 17 | `15-03` alerting that reaches a person | Now buildable: something finally collects the machine's own metrics |
| 18 | `16-01` personal-data map and residency | Fell out of the queue during a reshuffle rather than by decision. Small, and it is the file every other Stage 16 item reads before deciding what it must reach |
| 19 | `16-02` erasure: account and conversation | `8-07` above gives it a continuous consumer, which is how erasure stays known-working |
| 20 | `16-03` tenant data export | Its format is also `13-06`'s archive format, so the two want doing near each other |
| 21 | `16-04` the widget's processing notice | Plus the ADR on controller versus processor. The legal confirmation is `ago-business`'s, and does not block the mechanism |
| 22 | `16-05` personal data outside the database | Logs, traces, edge access-log retention, and the incident path |
| 23 | `17-03` secret inventory and rotation | Handling is already sound; rotation does not exist, and the visitor signing key is the expensive one |
| 24 | `17-05` runtime hardening | `securityContext` and `NetworkPolicy` everywhere. Limits blast radius rather than reducing the chance of a breach, which is why it is not higher |
| 25 | `10-03` console signup UI | Real only once `10-05` lands, which is in progress elsewhere |
| 26 | `11-07` finish the login theme | Error pages, and the anti-drift mechanism for the tokens that the first pass owed |
| 27 | `18-01` search across conversations | The one with real depth: a full-text index over a table partitioned twice, where the question is what a search may cost rather than which index type |
| 28 | `18-02` transfer a conversation | A contended change to the state `4-02` protects — two compare-and-sets that must agree, and `adr/0037`'s lock order to respect |
| 29 | `18-03` canned responses | Reuses `14-04`'s per-site scripted replies rather than inventing a second store of canned text |
| 30 | `18-04` internal notes and tags | Its correctness property: the visitor-facing read path must be incapable of returning a note, not merely filter it |
| 31 | `18-05` shortcuts and notifications | Extends `11-06`'s attention model; both defaults off, because a permission prompt on first load teaches people to click Block |
